...
function VeterinarianExaminations() {
  ...
  const [visibleCaretakerActiveRequests, setVisibleCaretakerActiveRequests] =
    useState(2);

  // Veterinarian visible requests
  const [visibleVeterinarianRequests, setVisibleVeterinarianRequests] =
    useState(2);
  ...
  function applyFiltersToRequests(
    ...
    if (nameVetFilter) {
      result = result.filter((request) =>
        request.veterinarianName
          ?.toLowerCase()
          .includes(nameVetFilter.toLowerCase())
      );
    }
    if (dateFrom) {
    ...
    if (statusFilter) {
      if (statusFilter === "InProgress") {
        result = result.filter(
          (req) => req.status === ExaminationStatus.InProgress
        );
      } else if (statusFilter === "Completed") {
        result = result.filter(
          (req) => req.status === ExaminationStatus.Completed
        );
      }
    }
          ...
          <>
            ...
            <div className="w-full max-w-[1024px] mx-auto mb-4 mt-4">
              ...
              <div className="flex flex-wrap gap-10">
                {cancelledRequests
                  .slice(0, visibleCancelledRequests)
                  .map((request) => (
                    <RequestCard
                      key={request.id}
                      request={request}
              ...
              {cancelledRequests.length > visibleCancelledRequests && (
                <div className="flex justify-center my-4">
                  <button
                    onClick={() =>
                      setVisibleCancelledRequests((prev) => prev + 2)
                    }
            ...
            <div className="w-full max-w-[1024px] mx-auto mb-4 mt-4">
              ...
              <div className="flex flex-wrap gap-10">
                {filteredCaretakerActiveRequests
                  .slice(0, visibleCaretakerActiveRequests)
                  .map((request) => (
                    <RequestCard
                      key={request.id}
                      request={request}
              ...
              {filteredCaretakerActiveRequests.length >
                visibleCaretakerActiveRequests && (
                <div className="flex justify-center my-4">
                  <button
                    onClick={() =>
                      setVisibleCaretakerActiveRequests((prev) => prev + 2)
                    }
                    className="w-20 h-8 border-2 border-main-blue text-main-blue rounded-full hover:bg-light-blue"
                  >
                    <span className="text-lg font-bold">...</span>
        ...
        {user.role === "Veterinarian" && (
          <>
            <div className="w-full max-w-[1024px] mx-auto mb-4 mt-4">
              ...
              <div className="flex flex-wrap gap-10">
                {filteredNewRequests
                  .slice(0, visibleVeterinarianRequests)
                  .map((request) => (
                    <RequestCard
                      key={request.id}
                      request={request}
              ...
              {filteredNewRequests.length > visibleVeterinarianRequests && (
                <div className="flex justify-center my-4">
                  <button
                    onClick={() =>
                      setVisibleVeterinarianRequests((prev) => prev + 2)
                    }
                    className="w-20 h-8 border-2 border-main-blue text-main-blue rounded-full hover:bg-light-blue"
                  >
                    <span className="text-lg font-bold">...</span>
