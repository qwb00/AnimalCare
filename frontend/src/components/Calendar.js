...
function Calendar({ selectedAnimalId  }) {
  ...
  // Fetch reservations for the selected animal
  useEffect(() => {
...
// Helper to format time slot intervals
          ...
          return (
            <div key={day} className="flex flex-col items-center p-2 border border-gray-300 bg-white rounded-xl">
              {timeSlots.map((slot) => {
                ...
                const isInactive = inactiveTimes.includes(slot) || isPastDay || isReserved;

                return (
                  <button
                    key={slot}
                    onClick={() => !isInactive && handleSlotClick(day, slot)}
                    title={isInactive ? (isPastDay ? 'Past date' : isReserved ? 'Already reserved' : 'Unavailable') : ''}
