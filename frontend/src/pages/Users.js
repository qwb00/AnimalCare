...
* Page for managing and creating users
*/
...
function Users() {
  ...
  // Conditions for edit role state
  const [editingUserId, setEditingUserId] = useState(null);
  const [originalRole, setOriginalRole] = useState("");
  const [editingRole, setEditingRole] = useState("");

  const roles = ["Caretaker", "Veterinarian", "Administrator"]; 
  const availableRoles = roles.filter((role) => role !== originalRole);
  ...
  // Editing mode for user
  const startEditing = (user) => {
    if (editingUserId === user.id) { // if already in mode close it
  ...
  const confirmRoleChange = async (userId) => {
    try {
      await axios.put(`${API_BASE_URL}/users/${userId}/role`, {
        newRoleName: editingRole
      }, {
        headers: {
          Authorization: `Bearer ${sessionStorage.getItem("token")}`
        }
      });
      console.log("Sending new role:", editingRole);
      // Reload user list
      await fetchFilteredUsers();
      setEditingUserId(null);
      setEditingRole("");
          ...
          {filteredUsers.map((filteredUser) => (
            <Card
              ...
              infoItems={[
                ...
                // if editing then showing select in other case text
                      ...
                      label: (
                        <div className="flex items-center">
                          ...
                          <img
                            ...
                            alt="Edit Icon"
                            className="h-5 w-5"
                            onClick={() => startEditing(filteredUser)}
                          />
                        </div>
                      ),
                      value: (
                        <div className="flex items-center space-x-2">
                          <select
                            ...
                            className="border border-gray-300 rounded-md p-1 focus:outline-none focus:ring-1 focus:ring-main-blue text-sm"
                          >
                            {/* Placeholder */}
                          ...
                          <Button
                            ...
                            onClick={() => confirmRoleChange(filteredUser.id)}
                          />
                        </div>
                      ),
                    }
