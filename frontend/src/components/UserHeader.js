...
function UserHeader({ user, updateUser }) {
  // Initialize avatar URL with user's photo or a default icon
  ...
  const { backgroundColor, borderColor, textColor } =
    roleStyles[userData.role.toLowerCase()] || {};
    ...
    const patchData = [
      {
        op: "replace",
        path: "/FirstName",
        value: updatedData.name.split(" ")[0] || "",
      },
      {
        op: "replace",
        path: "/LastName",
        value: updatedData.name.split(" ")[1] || "",
      },
