  ...
  return (
    <div className="w-full max-w-[1024px] mx-auto flex space-x-8 mt-6 mb-8 text-gray-500 text-lg">
      ...
      {itemsToShow.includes("Volunteers") && (
        <Link
          to={`/${userID}/volunteers`}
          className={getLinkClass(`/volunteers`)}
        >
          Volunteers
        </Link>
      )}
      {itemsToShow.includes("New Reservations") && (
        <Link
          to={`/${userID}/user-reservations`}
          className={getLinkClass(`/user-reservations`)}
        >
          New Reservations
        </Link>
      )}
      {itemsToShow.includes("Planned Walks") && (
        <Link
          to={`/${userID}/planned-walks`}
          className={getLinkClass(`/planned-walks`)}
        >
          Planned Walks
        </Link>
      )}
      {itemsToShow.includes("Medical Treatments") && (
        <Link
          to={`/${userID}/veterinarian-examinations`}
          className={getLinkClass(`/veterinarian-examinations`)}
        >
          Medical Treatments
        </Link>
      )}
      {itemsToShow.includes("Medical Prescriptions") && (
        <Link
          to={`/${userID}/medical-prescriptions`}
          className={getLinkClass(`/medical-prescriptions`)}
        >
          Medical Prescriptions
        </Link>
      )}
