...
function PlannedWalks() {
    ...
    const fetchPlannedWalks = async () => {
      try {
        ...
        if (filters.animalName)
          queryParams.append("AnimalName", filters.animalName);
        if (filters.animalBreed)
          queryParams.append("Breed", filters.animalBreed);
        if (filters.volunteerName)
          queryParams.append("VolunteerName", filters.volunteerName);
        if (filters.volunteerPhoneNumber)
          queryParams.append(
            "VolunteerPhoneNumber",
            filters.volunteerPhoneNumber
          );
        ...
        const upcomingWalks = data.filter(
  ...
  const handleMarkAsCompleted = async (id) => {
    try {
      ...
      if (!response.ok)
        throw new Error("Failed to mark reservation as completed");
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-sm font-semibold mb-2">
                  Time Range: {filters.timeRange[0]}:00 - {filters.timeRange[1]}
                  :00
                </label>
                <Range
                  step={1}
                  ...
                  onChange={(values) =>
                    setFilters((prev) => ({ ...prev, timeRange: values }))
                  }
            ...
            <Card
              ...
              infoItems={[
                ...
                },
                {
                  icon: icons.time,
                  label: "Time",
                  value: `${reservation.startTime.slice(
                    0,
                    5
